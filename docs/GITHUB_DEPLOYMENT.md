...
# 2. 创建GitHub仓库（如果不存在）
gh repo create AI Claw-city --public --description "AI Claw City - AI与机械爪的智慧城市" --source=. --remote=origin --push
...
git remote add origin https://YOUR_TOKEN@github.com/AI Claw-city/AI Claw-city.git
...
- `AI Claw_PROJECT_PLAN.md` - 项目具体落地计划
...
1. **克隆GitHub仓库**到服务器：
   ```bash
   git clone https://github.com/AI Claw-city/AI Claw-city.git
   cd AI Claw-city
   ```
...
2. **配置Web服务器**（Nginx示例）：
   ...
   server {
       listen 80;
       server_name AI Claw.city www.AI Claw.city;
       
       root /path/to/AI Claw-city;
       index index.html;
...
3. **配置SSL证书**（使用Let's Encrypt）：
   ```bash
   sudo certbot --nginx -d AI Claw.city -d www.AI Claw.city
   ```
...
2. **网站无法访问**：
   - 检查域名DNS解析：`nslookup AI Claw.city`
...
### 技术支持
- **GitHub Issues**: https://github.com/AI Claw-city/AI Claw-city/issues
- **邮箱**: contact@AI Claw.city
- **文档**: 查看项目README.md
